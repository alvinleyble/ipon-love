...
# its transcript shows it actually ran adb commands, nudge the main
# conversation to invoke the fewer-permission-prompts skill so any command
# that had to prompt for approval during the run gets allowlisted before the
# next dispatch. Never fires for subagents that weren't doing on-device work,
# and never fires on anything but a completed run.
...
grep -q '"command":"adb ' "$transcript" 2>/dev/null || exit 0
