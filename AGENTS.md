...
- JDK 21 (Android Studio JBR), `compileSdk = 37` (android-37.0 stable platform), `targetSdk = 36`. AGP 9.2.1 / Gradle 9.6 / Kotlin 2.2.10 (built-in via AGP).
- CI: `.github/workflows/unit-tests.yml` runs `./gradlew testStagingDebugUnitTest` (JDK 17) on every PR into `main` and every direct push to `main`. No signing/secrets — keep it that way.
...
**ADR-0009 — FK push order.** **The authoritative ordering is the `SyncTable` enum's declaration order (24 entries), frozen as [cross-platform-contract.md §3](docs/web/cross-platform-contract.md#3-fk-pushpull-order) — ADR-0009's original nine-table list is stale in *shape*, not just length, so do not use it.** Push is sequential in that order with per-table failure isolation; **pull is parallel**, so the ordering is a push guarantee. Upserts are idempotent by `id`; the per-table `server_rev > cursor` advance is post-commit, so an interrupted sync re-pulls rather than skips.
...
## Monetization
**One-time ₱249 purchase for Premium** (Google Play non-consumable in-app product — pivoted back to one-time on 2026-07-07 after a brief 2026-07-06 subscription plan; one-time won on the AI-cost and PH-market-adoption arguments). **Generous free tier** — recording your own money is *never* gated; Premium sells higher caps, cosmetics (extra palettes / future fonts), and delighters (calculator, recurring calendar, deep history). **Ads were dropped (2026-07-08, grill #5)** — no ad tier; `NO_ADS` stays only a dormant future option. The free-vs-premium split **and** the offline-first entitlement/gating architecture are **designed & grilled to build-ready** — couples-governance (either-partner-unlocks, via an `is_premium`/`premium_until` field on the synced `users` row), lapse policy (freeze — over-cap data goes read-only, never deleted), and entitlement caching are all resolved. The entitlement mechanism (client-trusted advisory column, cache-of-Play, fail-open cold-start, `entitlement_source` beta-comp guard) is formalized in **ADR-0044**; domain terms live in `CONTEXT.md` (`Entitlement`, `Enforcement`, `Freeze`, `Effective access`, `Cap count`, `Premium grant`). **Full spec: `docs/build/subscription-paywall-design.md`** (§8 feature map, §9 architecture D1–D8, §10 build spec, §11 grill-#5 mechanism seams G1–G10). Build stance: entitlement infra ships **dormant (kill-switch OFF, everything unlocked)**; enforcement flips on only after beta, on my explicit go.
