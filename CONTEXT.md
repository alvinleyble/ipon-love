...
**Receipt scan**:
Reading a receipt image — freshly photographed **or** picked from the gallery (a GCash/Maya confirmation screenshot counts) — into a **prefilled but unsaved** transaction form. Load-bearingly distinct from *attaching* a photo, which adds an image to a transaction that **already exists**: a scan fills a form still in progress and nothing is written until the user taps Save (or parks it — see [[Draft]]), and the two flows share only the `transaction_images` destination, not a code path. Recognition is classical on-device ML (bundled ML Kit, Latin), never a cloud call and never an LLM — so a scan works offline and costs nothing per use. **Reversed 2026-08-03 (ADR-0062 decision 6): scanning is now a fully paywalled `Feature.RECEIPT_SCANNING` gate** — convenience layered on recording, not recording itself; manual entry stays free and fully capable. The premium difference used to be what survives the scan rather than the scan itself (a free user's photo was read and dropped); that framing is superseded now that the scan itself is gated. See ADR-0062.
_Avoid_: OCR, receipt attach, import, photo upload

**Merchant memory**:
The inference of a transaction's [[Category]] and [[Account]] from the user's **own past transactions** whose `Note` matches the merchant name a [[Receipt scan]] just read — most-frequently-paired wins, tie-broken by recency. Deliberately **not** a hardcoded merchant→category table (which would know only PH chains someone maintains, and nothing about a neighbourhood sari-sari store) and **not** an LLM. It carries no storage of its own: the "memory" is the existing transaction history, and each scan feeds it by writing the cleaned merchant name into `Note`. Consequences worth naming rather than hiding: it infers **nothing** on the first scan at a new merchant, and it is the only part of a scan whose values appear **nowhere on the receipt** — which is why exactly these two fields are captioned in the editor while read fields are not. Own rows only; a partner's transactions are never consulted. See ADR-0062.
_Avoid_: auto-categorisation, smart category, learned rules, merchant table

**Draft**:
A user-parked, deliberately incomplete transaction, stored as its own row in `transaction_drafts` — not a flag on `transactions`, and not merely "a form the user hasn't saved yet" (that's a distinct, weaker thing — see [[Unfinished entry]]). A draft can legitimately have no amount, account, or category; it never counts in any balance, budget, or analysis total, is never visible to a partner, and syncs as a row (not its receipt photo, which stays local until promotion). Promoting a draft writes the real `transactions` row first, at the draft's own id, then retires the draft — never the other order. Free and uncapped; never auto-deleted. See ADR-0066.
_Avoid_: pending transaction, unsaved transaction, draft transaction, parked entry

**Unfinished entry**:
The in-memory-only, un-saved, un-drafted state of a **new** transaction form — distinct from [[Draft]], which is a deliberate, synced, indefinitely-kept parking action. An Unfinished entry survives navigating elsewhere in the app (so switching tabs and coming back resumes it) but lives only for the current app session: it is never written anywhere, and is gone for good on an explicit Discard or an app/process restart. There is at most one at a time, and it is invisible elsewhere in the app — no badge, no card, unlike Draft's pinned "Drafts (N)" card. Backing out of a genuinely new, never-parked transaction with something in it prompts Discard / Save as draft; picking Save as draft promotes it into a real Draft via the same `SaveDraftUseCase` the form's own button already uses. Scoped to new transactions only — Edit is unaffected, since an in-progress edit has a safe fallback (the original saved row) that a new transaction doesn't. See ADR-0068.
_Avoid_: in-progress editor state, in-flight entry, session state, pending entry

**Quick Add**:
The second entry point into transaction recording — a compact `ModalBottomSheet` launched directly from the home-screen widget as its own `ComponentActivity`, outside the app's main navigation graph. Deliberately **not** a shortcut into the full New Transaction form's state machine: it keeps its own smaller form state and reuses only the underlying use cases ([[Receipt scan]], [[Draft]] save/promote) the full form also calls, so it can gain scanning and drafting without regrowing into a second copy of the full editor (date picking, transfers, private/shared toggling, and manual receipt attach stay full-form-only). See v1.7.3 Item 14.
_Avoid_: quick entry, widget form, mini editor
...
**Entitlement**:
Whether a user currently holds Premium, carried as `is_premium` + `premium_until` on the synced [[Couple|users]] row (one-time purchase ⇒ `premium_until = null`). The column is a **client-maintained cache of that user's own Play Billing state**, reconciled on every foreground `queryPurchasesAsync`. Trust is **asymmetric**: for your *own* entitlement Play Billing is authoritative (the column only mirrors it); for your *partner's*, the column is authoritative and trusted unconditionally, since a device cannot query the other Play account. Freely **readable** (own row and, through the redacting partner view, the partner's), but **writable only through a single dedicated path** — the row's entitlement columns reject direct edits, so a client cannot set its own Premium by writing the column (ADR-0060). That path does not yet *verify* a purchase: it accepts what the client reports, so self-assertion is harder but not impossible, and because shared surfaces unlock on `me.active || partner.active`, one partner's spoof would unlock the joint features for both. Accepted as bounded because every gated item is cosmetic / a [[Cap count]] / ad-removal with **zero server cost**, and because [[Enforcement]] is still OFF. **Known prerequisite:** server-side purchase verification (Play RTDN) must land before *any* of — the first server-cost feature (AI companion), enforcement flip-day, or a web purchase path — whichever comes first. See ADR-0044 + ADR-0060.
