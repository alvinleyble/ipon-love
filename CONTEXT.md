...
**Receipt scan**:
Reading a receipt image — freshly photographed **or** picked from the gallery (a GCash/Maya confirmation screenshot counts) — into a **prefilled but unsaved** transaction editor. Load-bearingly distinct from *attaching* a photo, which adds an image to a transaction that **already exists**: a scan produces a draft and nothing is written until the user taps Save, and the two flows share only the `transaction_images` destination, not a code path. Recognition is classical on-device ML (bundled ML Kit, Latin), never a cloud call and never an LLM — so a scan works offline and costs nothing per use, which is why it is **never gated** despite being a convenience: it is recording your own money, just faster. The premium difference is what survives the scan, not the scan itself — a free user's photo is read and dropped (`maxReceiptPhotos = 0`), a premium user's is kept. See ADR-0062.
