...
/**
 * First-run graph (ADR-0024, currency step added by Item 27): Welcome → Currency → pair-or-solo
 * → starter-template picker → [onComplete]. Mounted by [com.iponlove.app.MainActivity] in place
 * of [IponApp] while the new-user gate is open; it has its own small back stack and is never
 * reachable once onboarding finishes (the caller swaps it out for [IponApp] rather than popping
...
fun OnboardingGraph(onComplete: () -> Unit) {
    ...
    NavHost(navController = navController, startDestination = WELCOME_ROUTE) {
        composable(WELCOME_ROUTE) {
            OnboardingWelcomeScreen(onContinue = { navController.navigate(CURRENCY_ROUTE) })
        }
