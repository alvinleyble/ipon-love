...
class FakeTransactionDao : TransactionDao {
    ...
    override fun observeForBalances(userId: String): Flow<List<TransactionEntity>> =
        changes.map {
            store.values
                .filter { !it.isDeleted && (it.userId == userId || !it.isPrivate) }
                .sortedWith(compareByDescending<TransactionEntity> { it.date }.thenByDescending { it.createdAt })
        }
    ...
    override suspend fun countByAccount(accountId: String): Int =
        store.values.count { (it.accountId == accountId || it.toAccountId == accountId) && !it.isDeleted }
...
fun transactionEntity(
    ...
    isSettlement: Boolean = false,
    isAdjustment: Boolean = false,
    transferFeeTransactionId: String? = null,
