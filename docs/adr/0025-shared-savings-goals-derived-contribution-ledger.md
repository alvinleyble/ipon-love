...
5. **Unpair leaves benign orphans.** Your own contributions to an ex-partner's goal are left (owned by you, not purged). They're invisible (the goal is gone) and the calculator ignores contributions whose goal is absent, so they never leak into a sum. Not soft-deleted — that would erase real history from the partner's now-personal goal.
   - **F3 addendum (2026-07-03).** Because those orphan rows persist on the owner's goal, re-sharing that *same* goal into a **different** couple would resurface the ex-partner's amount + note to the new partner (view gated only on the goal's *current* `couple_id`, not on contributor membership) and re-add it to the derived total on both devices. Fix: `partner_goal_contributions` also nulls content when the contributor is no longer a member of the goal's couple (`gc.user_id not in (select id from users where couple_id = g.couple_id)`) — folded into the same purge path (null amount → `isDeleted`). Same-couple unshare→reshare is unaffected (the partner is still a member). No client change; migration `2026-07-03_partner_goal_contributions_membership.sql`.
