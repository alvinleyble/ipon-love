...
1. **Native, not redirect (Q1).** `androidx.credentials` + `com.google.android.libraries.identity.googleid` drive an on-device credential request (raw nonce + its SHA-256 hash, per the documented security step); the returned ID token + raw nonce go to Supabase's `signInWith(IDToken)`. The OAuth-redirect/Custom-Tab flow is **rejected for login** — worse UX (browser hop, web account picker) for no gain, since the deep-link plumbing it would reuse isn't a bottleneck.

   **Build-time correction (2026-07-24, on-device verification):** the credential primitive is **`GetSignInWithGoogleOption`**, not `GetGoogleIdOption` as this ADR originally specified. On-device, `GetGoogleIdOption` threw `NoCredentialException` ("No credentials available") *even with valid Google accounts on the device and correct config* — its bottom-sheet/one-tap mechanism (GMS `GoogleIdService` returned `CANCELED`) is a documented failure mode. `GetSignInWithGoogleOption` is the primitive purpose-built for a **button-triggered** "Sign in with Google" tap: it always launches the full account chooser and returns the same `GoogleIdTokenCredential`. This is a strictly closer fit to decisions 5 and 7 (button, picker always shows, explicit action) — so the switch *reinforces* the design rather than changing it. Same nonce handling, same downstream wiring.
...
5. **No auto-select; sign-out stays method-agnostic (Q6/Q9).** The chosen `GetSignInWithGoogleOption` (see decision 1's build-time correction) always shows the account picker, so "Continue with Google" is always an explicit action — never a silent auto-login. That removes any need for `clearCredentialState()` plumbing on sign-out; the existing sign-out (clear session + `LocalDataWiper`) already keys off session teardown and needs no Google branch.
...
- **`GetGoogleIdOption`** (the automatic one-tap / bottom-sheet credential option) — originally specified here, but it threw `NoCredentialException` on-device with valid accounts present (GMS `GoogleIdService` → `CANCELED`); replaced by `GetSignInWithGoogleOption`, the button-flow primitive, during 2026-07-24 verification (see decision 1).
