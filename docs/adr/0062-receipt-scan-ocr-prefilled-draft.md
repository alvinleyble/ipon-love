...
- **No schema change, no Room change (stays v31), no Supabase migration**, and **no impact on [Item 1](../build/v1.7.3.md#item-1--freeze-the-cross-platform-contract-from-web-phase-0-w2)'s cross-platform contract freeze** — the parser is client-local, mints no deterministic ids, and writes only through existing synced columns.
