...
**V1 through V1.6.4 are committed on `main`** (latest code: `0664f49` note attachment cap). Per-item detail for every shipped batch lives in the version docs indexed below — this section tracks only what's *in flight*.

**In flight: [V1.6.5](v1.6.5.md) — multi-image receipts (the V1.6.4-deferred follow-up).** Code `DONE a19e910`, build green (`testStagingDebugUnitTest` + `assembleStagingDebug`), **not yet on-device verified, and the staging migration is not yet applied by Alvin.** Replaces the single `transactions.attachment_url` with an up-to-3 `transaction_images` child table mirroring notes end-to-end (Room v23, backfill-then-drop migration, partner redacting view + SECURITY DEFINER Storage RLS reusing the `receipts` bucket, swipe-scrollable strip UI + combined-view thumbnail). Two forks grilled first: editor-state list reconciled on save (deferred persistence, no orphan child rows), and backfill-then-drop the column (dev-phase). **Before ship: Alvin applies `2026-07-07_transaction_images.sql` to staging + on-device verifies.**
...
| V1.6.4 | Reactive fix batch (not booked/grilled), **complete 2026-07-07**. (1) Sign-out crash + permanent "unpaired" wedge — `LocalDataWiper` cursor/Room reorder + 7-repo eager-`userId()` crash class + `WatchUnpairUseCase` race, `DONE 804da03`; (2) photo attachments upload but never display — private-bucket auth loading (`StorageAuthInterceptor`) + broken partner-read RLS rebuilt on `SECURITY DEFINER` (ADR-0043) + tappable note-image viewer + combined-view partner receipt thumbnail, `DONE 182e463`; (3) cap note attachments at 3, `DONE 0664f49`. Multi-image-receipts follow-up scoped but deferred to V1.6.5 | [v1.6.4.md](v1.6.4.md) |
| V1.6.5 | Multi-image receipts (the V1.6.4-deferred follow-up), started + grilled + built 2026-07-07. Single `attachment_url` → up-to-3 `transaction_images` child table mirroring notes end-to-end (Room v23, backfill-then-drop, partner redacting view + SECURITY DEFINER Storage RLS, swipe strip UI + combined thumbnail). Code `DONE a19e910`, build green — **pending Alvin's staging migration apply + on-device verify** | [v1.6.5.md](v1.6.5.md) |
...
15. **Subscription paywall + feature gating** — pivot in the monetization model, decided 2026-07-06: Love, Ipon will monetize via recurring Google Play subscription billing, not the one-time-purchase plan `CLAUDE.md`/`PRD.md` previously described. Greenfield, not yet designed/grilled — needs a decision on Google Play Billing Library integration, entitlement checking/caching (offline-first complicates this — the app must still be usable without a live billing check on every launch), and critically **which features sit behind the paywall vs. stay free** (candidates floated informally: extra theme palettes beyond a free subset, in-app calculator, custom fonts once built, shared-account/shared-category counts beyond a free cap, budget period stepper granularity, the "paid for partner" debt feature, savings-goal count beyond a free cap — none of these are locked, just brainstormed). **Full design & grilling scaffold: [subscription-paywall-design.md](subscription-paywall-design.md)** — the two hardest, app-specific open questions are the *lapse policy* (can't delete a user's financial data when they drop below a cap) and *couples governance* (whose entitlement unlocks shared features, and how an offline device learns the partner subscribed). Proposed sequencing: build the gating **infrastructure next (dormant, kill-switch off)** *ahead of* most other Horizon items so unbuilt paid-candidate features are born gate-aware, and flip enforcement on only after beta. **Target: not yet determined** — internal-only for now, not on the tester-facing Upcoming features screen until the design is grilled.

Added 2026-07-05 (items 10-12, requested by Alvin alongside the V1.6.1 additions below; item 13 deferred from V1.6.2 same day; item 14 deferred from V1.6.3 same day). Item 15 added 2026-07-06 (monetization pivot to subscription). **2026-07-05 addendum, revised 2026-07-06:** provisional public target quarters assigned above for the subset surfaced on the tester-facing Settings → Upcoming features screen (items 1, 2, 5, 7, 8, 9, 13, 14 — all **Q3 2026**); items 3 (AI companion), 4 (Password vault), 6 (iOS), and 15 (subscription paywall) stay explicitly **not yet determined**; items 10-12 stay internal-only and don't appear on that screen. These are planning targets, not commitments, and can move — this doc is the single source of truth for them; `PRD.md`/`ARCHITECTURE.md` list the same 15 items without duplicating the dates, and `UpcomingFeaturesScreen.kt` mirrors the tester-facing subset above.

---

## July 2026 Build Roadmap (provisional)

**Build order for the Horizon items, decided 2026-07-06. Build starts Jul 7.** Schedules **13 of the 15** Horizon items — **#4 (Password vault) and #6 (iOS/KMP) are deferred out of the July runway** (no demand signal / evaluation-not-a-slice respectively); they stay on the Horizon list above, unscheduled.

**Sequencing rules:** (1) **#15 leads** — subscription gating infra must land before any feature it gates (#8 custom fonts; it also informs #3 AI's billing). (2) Its grill and #3 AI's grill are the long poles, so **both grills start day 1 and run in parallel** with the early build weeks — grill each item to resolution *before* building it (`/grilling`), especially #15 (see [subscription-paywall-design.md](subscription-paywall-design.md)) and #3. (3) Non-gated items fill the remaining weeks.

| Week | Dates | Items | Notes |
|---|---|---|---|
| 1 | Jul 7–13 | **#15** subscription gating infra (grill → build dormant, kill-switch OFF) · **#13** restart-fresh · **#14** display-currency symbol | #15 is the substantial work; #13/#14 are already-scoped no-grill fillers. **Kick off #15 + #3 grills day 1.** |
...
16. `2026-07-07_transaction_images.sql` ([V1.6.5](v1.6.5.md), code `a19e910` — `transaction_images` child table + trigger/indexes/owner RLS + `partner_transaction_images` redacting view; **backfills** each non-null `attachment_url` into one row then **DROPs** the column (+ rebuilds `partner_transactions` without it); re-points `partner_can_read_receipt` at `transaction_images`. **NOT yet applied to staging — Alvin applies with the app update; destructive on `attachment_url`.**)

All confirmed applied live (#13/#14 verified 2026-07-06 via PostgREST column probes; #8/#12 were re-verified 2026-07-05 after they'd silently drifted out of the "applied" record despite being real, committed, shipped features — #12 in particular meant every rollover-enabled budget save had been failing to push since `50d9a92`; #15 applied and re-tested live by Alvin 2026-07-07). **#16 is NOT yet applied** (V1.6.5, code `a19e910`). `supabase/schema.sql` remains the authoritative end-state.
