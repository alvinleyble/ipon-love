...
**In flight — the premium paywall build (Horizon #15),** following the [subscription-paywall-design.md](subscription-paywall-design.md) §12 slice plan (the resume-able tick-box build order). **Phase 0 · Item 2 (Records future-month cap) DONE:** the Records + Combined month steppers now cap forward paging at the current month via a shared, unit-tested `MonthWindow.canStepForward` predicate + a `canGoNext` flag on `MonthStepperRow` (Analysis/Budgets/Recurring left steppable by design). Also closes [v1.6.5.md](v1.6.5.md) Item 2. No Room/schema change. **Phase 0 · Item 3 (Analysis) grilled + re-scoped 2026-07-09:** the 2026-07-08 attempt to *build* new trailing 3m/6m/All ranges was **reverted** (a separate row was redundant beside the existing calendar 3M/6M/12M/ALL tabs); the premium lever is now a **wire-only soft-gate on those existing tabs** (free = 1D/1W/1M → Phase 2 S10). The remaining Item 3 *base-behaviour* work was grilled to build-ready: generalize the **Flow** sub-tab to all ranges + give the **Calendar** sub-tab 1M-coupled month-stepper behaviour (shared-anchor forward-cap + −12/−13 `DEEP_HISTORY` gate) — full spec in [v1.6.5.md](v1.6.5.md) Item 3. **Next unchecked build:** §12 Phase 0 · Item 3 base behaviours (Flow + Calendar), then Item 4 (Notes char-limit).
