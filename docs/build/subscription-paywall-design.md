...
| **Add txn — receipt scan (OCR)** | ✗ (entire feature) | ✓ | Hard gate — new `Feature` entry (TBD name) | **ADDED 2026-08-03** (captain's ruling, superseding [ADR-0062](../adr/0062-receipt-scan-ocr-prefilled-draft.md) decision 6's original "not gated"). Both scan entry buttons — camera and gallery — gate; manual entry and the existing attach flow are untouched. Governing reading is not D5's "own-money recording is never gated" but the one already used for voice: convenience layered on recording, not recording itself. `maxReceiptPhotos` (row above) no longer differentiates scanning — it still governs manually attached photos only. Mechanism is ADR-0044's; gate ships **dormant** per the standing build stance. See ADR-0062's amendment note for the full reversal record. |
...
| **Couple — banner photo** | derived accent gradient (Item 9) | **+ uploaded couple photo** | Soft gate (`COUPLE_BANNER_PHOTO`), **shared** scope, revert-on-lapse | **ADDED 2026-07-24** (v1.7.0 Item 10, grilled). The **first shared soft gate** — either partner's premium unlocks it (D1). Freeze is a pure read-time derivation (`effectiveBannerSource(url, unlocked)`): lapse reverts to the Item 9 gradient, re-grant auto-restores the photo, object retained (§2.1). Non-cannibalizing: the free layer (gradient) is always present. |
...
| **Records — transaction drafts** | **free, uncapped** | same | **none — deliberately ungated** | **ADDED 2026-08-05** (v1.7.3 Item 8, grilled — [ADR-0066](../adr/0066-transaction-drafts-parking-area.md) decision 8). Recorded here so it isn't re-litigated: **no `Feature` entry and no `PlanLimits` field**, so §10.1's locked contract is untouched. Alvin's ruling: *"Drafting a transaction is free, but receipt isn't, so there should be no friction there."* The premium differential already sits upstream — receipt **scanning** is fully paywalled (ADR-0062 decision 6, reversed), so a free user simply never produces a draft *with* a photo; a second gate on the drafts screen would be redundant. **A cap was considered and rejected:** it would block the "just park it" flow that *is* the feature, and D5's "recording your own money is never gated" applies squarely. Pileup is handled by a passive Records card + per-row age label (ADR-0066 decision 10), never a wall and never auto-deletion. |
...
| `COUPLE_BANNER_PHOTO` | soft — tap "Set couple photo" while locked → paywall; the derived gradient (Item 9) is the always-present free layer | Settings → Couple + Combined view | **shared** (the first shared soft gate — D1) | **WIRED (dormant) `c4bd35b`** — `Feature.COUPLE_BANNER_PHOTO` added, consulted via `PremiumGate.observeLocked(Scope.SHARED)` in `CoupleViewModel` + `CombinedViewModel` (v1.7.0 Item 10). Render is a pure read-time `effectiveBannerSource(url, unlocked)` derivation (freeze/re-grant non-destructive, like G8 palettes). Zero effect until the enforcement flip. |
...
- [x] Item 3 — Analysis **Flow-generalization + Calendar month-stepper** base behaviours ([v1.6.5.md](v1.6.5.md) Item 3; grilled 2026-07-09) — **DONE `97cf5c7`**. *(The premium range lever is now a wire-only soft-gate on the existing 3M/6M/12M/ALL tabs → moved to Phase 2 S10; the 2026-07-08 "rolling ranges" build was reverted.)*
- [x] Item 4 — Notes char-limit *existence* ([v1.6.5.md](v1.6.5.md) Item 4) — **DONE `6e95238`** (2026-07-11, on-device verified; base ceiling = premium max 50k, ships dormant; the 5k/50k split is S10). **This closes Phase 0.**

**Phase 1 — dormant infrastructure (enforcement OFF, nothing locks):**
- [x] **S1 — Schema migration** — **DONE `1ef42b7`** (2026-07-10, applied + verified on staging): `users` entitlement columns (`is_premium`, `premium_until`, `entitlement_source`, `entitlement_checked_at`); `app_config` row (`enforcement_enabled` + `cap_overrides` jsonb) cached in Room via a new `core/config` slice (fail-open `DORMANT` fallback, G4). Room v23→v24 (plain `AutoMigration`); mapper round-trip + app_config-cache JVM tests. **Finding:** there is **no `partner_users` redacting view** — partner user rows are read straight off the base table via the `users_select` same-couple policy, so the four columns are partner-visible automatically (exactly what D2/ADR-0044 want; nothing to project). Per-user grants live on the `users` row (`entitlement_source='GRANT'`), **not** in `app_config` (ADR-0044 §4), so `app_config` carries only the kill-switch + cap overrides.
- [x] **S2 — `core/entitlement` domain** — **DONE `2ef15f7`** (2026-07-10): `Feature` enum (6 boolean gates, no per-feature scope — G6 resolves scope from the gated row at the call site), `Entitlement`/`EntitlementSource` (1:1 map target for the S1 `users` columns; `isActive(now)` honors nullable `premiumUntil`), `EffectiveAccess` (`hasAccess` = `me.active || partner.active` for SHARED / self for INDIVIDUAL, null partner → self; `shouldLock = enforcement_enabled && !hasAccess`), `PlanLimits` (§10.1 FREE/PREMIUM tables + `resolve(hasAccess, overridesJson)`), `CapCount` (block-on-create `currentCount >= limit`, G1/T1). JVM tier-1 tests green. *(Built on Sonnet, not Opus as the plan suggested — the work was pure from-spec translation with no open design calls; an Opus review pass then caught + fixed one real gap: the `cap_overrides` shape is now tier-scoped `{"free":{…},"premium":{…}}` so a free-tier tuning can't bleed onto premium users.)* All dormant — nothing in the app references it yet (S4 reconcile + S7+ gates consume it).
- [x] **S3 — `core/billing`** — **DONE `fcfae8a`** (2026-07-10): `BillingGateway` interface (`queryOwnedPurchases`, `acknowledge`) + real `PlayBillingGateway` over Play Billing Library **v9.1.0** (`billing-ktx`, added to the version catalog) + Hilt binding. `queryOwnedPurchases` doubles as both the silent reconcile check (S4) and the user-facing "Restore purchases" action (S5) — Play has no separate restore API for non-consumables (§3: restore is automatic via `queryPurchasesAsync` on a new device signed into the same Play account). **Scope note:** launching the purchase flow (`launchBillingFlow`) is deliberately *not* in this wrapper — deferred to S5, since it needs an `Activity` and is naturally owned by the screen presenting it. API surface (`queryPurchasesAsync`/`acknowledgePurchase` KTX suspend extensions, `PendingPurchasesParams.enableOneTimeProducts()`, `enableAutoServiceReconnection()`) verified against the decompiled `billing-ktx:9.1.0`/`billing:9.1.0` classes via `javap`, not guessed from docs. Dormant + unwired — nothing injects `BillingGateway` yet, so no `BillingClient` connection ever opens at runtime (no on-device surface); verified via `assembleStagingDebug` (full Hilt/Dagger graph + manifest merge, confirms the `com.android.vending.BILLING` permission auto-merges) + the full JVM suite green. No dedicated unit test for `PlayBillingGateway` itself (thin real-SDK wrapper, no logic — same precedent as `SupabaseUserRemoteSource`/`SupabaseAppConfigRemoteSource`).
- [x] **S4 — Entitlement reconcile loop** — **DONE `40aa2bc`** (2026-07-10, on-device verified): the runtime layer tying S1–S3 together, and the **first paywall slice live in the sync path**. A new `FullSyncStep` hook on `SyncEngine` runs **only in the full `sync()` path** (foreground/reconnect/WorkManager), never on the debounced `pushOnly`/bell `pullOnly` — so the Play-billing IPC (G2 cache-of-Play) + `app_config` refresh ride the sync trigger but don't fire on every keystroke-push; each step is engine-guarded so a Play/network failure can't abort the data sync. `core/entitlement/EntitlementRepository` = `observeSelf`/`observePartner` (partner column read straight off the replicated `users` row — no separate view, S1) + `reconcile()`: **G7 GRANT-skip** (returns *before* querying Play so a beta comp is never wiped — the §10.7 flip-flop trap), offline-tolerant no-write on billing failure (§10.4 last-known-good), acknowledge-unacked, and an **idempotent only-on-change write** so a steady-state foreground never dirties the users row. **G4 fail-open cold-start** is already satisfied by S1's `AppConfig.DORMANT` fallback + `is_premium=false` default — `AppConfigSyncStep` now refreshes it on the sync trigger so enforcement self-heals. Two `FullSyncStep`s multibound (`EntitlementReconcileStep`, `AppConfigSyncStep`); row mechanics + `toEntitlement` mapper on `feature/user`'s `UserRepository`, policy in `core/entitlement`. **The realtime accelerator needs no new code** — the existing couple bell + `observePartner` already carry the column (D2); the send-side bell fires when S5 calls `reconcile()` directly post-purchase (outside a full sync). 10 tier-1 reconcile state tests; no Room/schema change (v24). On-device verified (testdev17, real device): staging GRANT replicated into local Room, `app_config` cached, G7 skip fired every foreground, nothing user-visible.
- [x] **S5 — Paywall screen** (`feature/subscription`) + **Item 12** dormant Settings entry + Restore — **DONE `64c769b`** (2026-07-10, on-device verified): the first *user-facing* paywall slice. `feature/subscription` (SubscriptionScreen/ViewModel/UiState) — self-only "already owned" state (§10.3), benefit list, ₱249 CTA, Restore, Privacy Policy link. `core/billing` gains the purchase-flow launch S3 deferred (needs an `Activity`): `PurchaseResult` + a hot `purchaseResults` SharedFlow + `launchPurchaseFlow()`; `PlayBillingGateway` wires the `PurchasesUpdatedListener` + a v9 offer-token-aware `ProductDetails` query, failing fast with `ITEM_UNAVAILABLE` while the Play product doesn't exist (pre-S11). On success the VM **reconciles rather than trusting the callback** (writes+pushes the users column → partner unlock D1/D2; `observeSelf` flips the UI to active with no restart); Restore = `reconcile()` then found/not-found. The Settings **Premium** row (Item 12) is **enforcement-gated** — hidden while dormant, visible only when `enforcement_enabled` flips ON (which *is* the "explicit go", §10.7) — derived in `PersonalizeViewModel` from `AppConfig`+own entitlement; route wired into the SETTINGS nav graph. No gates wired yet (S7+), no Room/schema change (v24). 8 tier-1 `SubscriptionViewModel` tests + `FakeBilling` extended for the new members. On-device (A142): enforcement-ON → row + paywall + Restore against real Play; enforcement-OFF → row gone. *(Built on Opus.)*
- [x] **S6 — Analytics primitive** (`core/analytics`) — **DONE `b2c3565`** (2026-07-10): Room-buffered, **push-only** `analytics_events` flush on the full-sync trigger (G10 / §10.10, **no third-party SDK**). `Analytics.log(name, source, params)` fire-and-forget buffer write (no-op with no session) + `AnalyticsFlusher.flush()` drain — idempotent **upsert-by-`id`**, delete-on-land, **offline-keep**, 500-row trim. `AnalyticsSyncStep : FullSyncStep` (full sync only, engine-guarded — the S4 pattern; never a keystroke `pushOnly`). New `analytics_events` table (client-UUID PK, `user_id` FK, own-row insert+select RLS — the self-select lets the default `returning=representation` upsert succeed; **migration #18 applied + verified on staging**). **Room v24→v25** (`AutoMigration`). S5 `SubscriptionViewModel` wired as the first consumer (`paywall_impression`/`purchase_started`/`purchase_success`/`purchase_cancelled`/`restore`); gate-touchpoint `upsell_tap` deferred to the Phase 2 gates. Ships **dormant** (flush runs, buffer empty until the paywall is reachable). 11 tier-1 JVM tests (`RoomAnalyticsTest` + `SubscriptionViewModelTest` funnel cases); `testStagingDebugUnitTest` + `assembleStagingDebug` green. *(Built on Sonnet — pure from-spec pattern-following.)* **This closes Phase 1.**

**Phase 2 — wire gates (still dormant; verify each by flipping enforcement ON in staging):**
- [x] **S7 — Count-cap gates, grouped by mechanism** — **DONE `c57505a`** (2026-07-10, on-device verified): the pattern-setting gate slice. `core/entitlement/PremiumGate` + `CapCheck` (enforcement + self/partner entitlement + `PlanLimits.resolve` → one create-time decision; dormant fast-path short-circuits to `Allowed` before reading a limit) + shared `core/ui/CapReachedSheet`/`UpsellPrompt` (the one upsell surface, routes to the paywall, logs `upsell_tap` §10.10). Eight caps via `Check…CapUseCase`s owning the G1 count: personal/shared accounts + categories (personal @ create, shared @ share-transition), budgets (personal, **per-month** — budgets are inherently monthly, and the duplicate-to-next-month action is gated too so it can't bypass), savings goals (personal @ save, shared @ toggle), couple debt (SHARED, un-settled count off the board). `onOpenPremium` threaded through ManageScreen/CoupleScreen/GoalEditorScreen. Block-on-create + `Freeze` confirmed (over-cap data stays visible + read-only). No Room/schema change (v25). `PremiumGateTest` (9 cases) + fakes updated; JVM + `assembleStagingDebug` green. On-device verified (testdev17, free + paired) via a staging enforcement flip. *(Built on Opus.)*
- [x] **S8 — Media caps** — **DONE `feb2d23`** (2026-07-11, on-device verified): the two per-instance media caps wired into their add paths, reusing the S7 mechanism verbatim (`PremiumGate.checkCap` → `CapCheck` → shared `CapReachedSheet`/`UpsellPrompt` → `onOpenPremium` route + `upsell_tap` §10.10). `CheckReceiptPhotoCapUseCase` (`maxReceiptPhotos`, INDIVIDUAL — free 1 / premium 3) gates `AddTransactionViewModel.onImagePicked`; `CheckNoteAttachmentCapUseCase` (`maxNoteAttachments`, INDIVIDUAL — free **0** / premium 3, so the *first* attachment is blocked when enforced) gates `NoteEditorViewModel.addImage`. Unlike the S7 entity-count gates these own **no DAO query** — the count is the editor's in-flight image count (transient per-editor UI state), passed in per the gate's G6 count-on-the-caller contract. The existing hard `MAX` (=3, = the premium max) stays as the dormant/premium backstop; the gate only limits the free tier, so dormant behavior is byte-identical to before. `onOpenPremium` threaded through both AddTransaction routes (add + edit) and the NoteEditor route. No Room/schema change (v25). 7 tier-1 JVM cases across two test files (dormant bypass, free boundary incl. the note first-attachment block, premium bypass); `testStagingDebugUnitTest` + `assembleStagingDebug` green. On-device verified (testdev17) via a staging enforcement flip. *(Built on Sonnet — pattern-following the S7 gate, no open design calls.)* **Flip-day polish (noted, not fixed):** the upsell fires *after* the system photo picker closes (the picker launches from the button; only the chosen URI reaches the gate), so an enforced free user picks a photo, then sees the cap sheet — functionally correct (nothing added, T1 freeze) but a picker-then-upsell ordering wart. Left as-is for the dormant slice rather than restructuring the picker flow in both screens; revisit at flip day if it bites.
- [x] **S9 — Boolean soft gates** — **DONE `a4ff11d`** (2026-07-11, on-device verified): palette allowlist + revert-reconciliation (G8), calculator, budget-rollover toggle, recurring-calendar blur, all still dormant. New reactive seam `PremiumGate.observeLocked(scope)` (`enforcement && !hasAccess`, no `PlanLimits` read) drives all four; `Scope.INDIVIDUAL` throughout. **G8 is a pure read-time derivation** (`ThemePalette.effective(locked)` → free default) applied in `MainActivity` + the Personalize preview — the chosen palette is never written down, so a re-unlock auto-restores it non-destructively (cleaner than a write-based reconcile). Calculator = shared new `core/ui/FeatureLockedPanel` wall; budget rollover = locked toggle (an enabled budget keeps rolling, T1 freeze); recurring calendar = blurred preview, list + rule creation stay free (§8.2). Each locked tap logs `upsell_tap`. No Room/schema change (v25). 10 tier-1 JVM cases (5 `observeLocked`, 5 `ThemePalette.effective`). *(Built on Opus per the flag — G8 + the seam were the novel part; the other three rode it mechanically.)*
- [x] **S10 — Build-then-gate splits** (three sub-gates, separate commits, lightest first) — **ALL DONE 2026-07-11; closes Phase 2:**
  - [x] **sub-gate 1 — `maxNoteChars` 5k/50k split** (on Item 4) — **DONE `f0da7e4`** (2026-07-11). New reactive `PremiumGate.observeLimit(scope, limitOf)` resolves the note-editor ceiling (free 5k / premium 50k, individual); dormant = premium max = byte-identical. **Freeze-safety add** (`NoteCharLimit.effectiveLimit = max(tierLimit, seededLength)`) freezes an existing over-cap note rather than truncating it on flip day (T1 / §10.7). [v1.6.5.md](v1.6.5.md) Item 4 "S10 split". *(Built on Opus — real freeze design call.)*
