...
- [ ] **S10 — Build-then-gate splits** (three sub-gates, separate commits, lightest first):
  ...
  - [ ] **sub-gate 2 — `ANALYSIS_EXTENDED_RANGES`** (soft-gate the *existing* 3M/6M/12M/ALL tabs — free = 1D/1W/1M) — wire-only via `observeLocked`.
  - [ ] **sub-gate 3 — `DEEP_HISTORY`** (Records Item 2 + Analysis 1D/1W/1M shared anchor, −12/−13 wall) — the one bit with new behavior.
