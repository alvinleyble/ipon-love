...
| 24 | `TRANSACTION_DRAFTS` | owned leaf, own-user-only, never a money row |
...
- **`NOTIFICATIONS` and `TRANSACTION_DRAFTS` are the two trailing entries** because nothing depends on either and neither must ever delay a financial row's push (ADR-0053, ADR-0066). `TRANSACTION_DRAFTS` was appended after `NOTIFICATIONS` on that same argument, which is why ordinals 1–23 were unaffected by adding it.
...
#### ✅ Amendment landed — the 24th entry, `TRANSACTION_DRAFTS`

**Booked 2026-08-05, built 2026-08-06** ([ADR-0066](../adr/0066-transaction-drafts-parking-area.md) / [v1.7.3 Item 8](../build/v1.7.3.md#item-8--transaction-drafts-a-parking-area-inside-records-third-exit-from-the-new-transaction-form)). `transaction_drafts` is now `SyncTable` #24 in code, **appended after `NOTIFICATIONS`** — a pure append, so **ordinals 1–23 are unchanged** and no other client needs to re-derive anything.

For a porting client: `transaction_drafts` is **owned, own-user-only** (RLS `user_id = auth.uid()`, no redacting view — a partner can never see a draft), **every content column is nullable** (a draft is a partial form: no `type`, `amount`, `account_id` or `category_id` is required, and there is no FK on the id columns), rows carry ordinary **v4 random** ids per §1.5, and **promotion to a real transaction writes the `transactions` row *before* retiring the draft** (the draft's id *is* the transaction's id, so a re-run is an idempotent upsert; the reverse order loses data). A draft never counts as money anywhere — by construction, since it is not in the ledger table. §1, §1b, §5.3 and §9 are unaffected — checked, not overlooked.

One deliberate asymmetry a web client must render rather than resolve: the draft **row** syncs but its **receipt photo does not** (it stays a local file until promotion), so a draft authored on the phone arrives on web with a `receipt_count` and no image.
...
- [x] **§3 FK order** — ✅ FROZEN 2026-08-05: the `SyncTable` ordering (supersedes ADR-0009's nine); push sequential with per-table failure isolation, pull parallel; per-table `server_rev > cursor`, page 500, post-commit advance. ✅ *Amended 2026-08-06: a 24th entry (`TRANSACTION_DRAFTS`) appended after `NOTIFICATIONS`, ordinals 1–23 unchanged — see §3.1.*
