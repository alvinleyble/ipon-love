...
-- ---------- notifications (own-user-only inbox) ------------------------------
-- The synced source of truth for every notification the app raises; the OS push is a
-- best-effort courtesy on top ("Way A" — inbox-primary, ADR-0053). Own-user-only: RLS is
-- own-row, there is NO partner redacting view, and the table is never replicated. Item 9's
-- "partner logged a debt" notification lands as the RECIPIENT's own row, generated by their
-- own client from the already-synced debt.
--
-- `id` is TEXT, not uuid: a deterministic composite key derived from the event itself
-- (`budget:{budget_id}:{yyyy-MM}:{slot}`, `recurring:{occurrence_id}`, `debt:{debt_id}`), so the
-- same event detected independently on two clients merges instead of duplicating. Clients
-- generate create-if-absent, never upsert-over, so a re-detected event can't overwrite
-- read/dismissed state — the row's existence IS the dedup record.
--
-- Retention: rows older than 60 days are HARD-deleted by the client sweep — a narrow,
-- sanctioned exception to ADR-0010's tombstones-only rule, resurrection-safe because every
-- client computes the same cutoff. Dismiss / clear-all stay ordinary soft-deletes.
...
-- a notification about the partner's activity is generated locally into the
...
--  Single-row, public-read config table. NOT a synced entity — no server_rev/
--  updated_at/pending_sync, never touched by TableSyncer push/pull (ADR-0009
--  doesn't apply here). Alvin bumps required_version_code by hand (Supabase
--  table editor, which writes as the postgres role and bypasses RLS) before a
--  new build reaches testers. No insert/update/delete policy for clients —
--  this table is never written from the app itself. `id boolean primary key
--  default true` + `check (id)` is a standard Postgres singleton-row trick:
--  only the value `true` can ever satisfy the primary key, so at most one row
--  can ever exist.
-- ============================================================================
